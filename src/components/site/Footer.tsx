    ...
    <footer className="border-t border-border/60 bg-[color:var(--surface)]">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-5">
        <div>
          <div className="flex items-center gap-2 font-display text-lg font-bold">
            <span className="grid h-8 w-8 place-items-center rounded-xl gradient-brand text-white">M</span>
            MyRoomiee
          </div>
          <p className="mt-3 text-sm text-muted-foreground">
            Managed PG, coliving and furnished accommodation across Mumbai. Zero brokerage, verified properties, premium living.
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li><Link to="/pg-in-malad-east" className="hover:text-foreground">PG in Malad East</Link></li>
      ...
      <div className="border-t border-border/60">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-5 py-5 text-xs text-muted-foreground md:flex-row">
          ...
          <p>Managed PG • Coliving • Furnished Accommodation • Mumbai</p>
        </div>
      </div>
    </footer>
  );
