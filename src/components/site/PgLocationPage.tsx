...
const amenitiesGrid = [
  ...
  { icon: Flame, name: "Gas Connection" },
...
const propertyHrefForRoom = (roomType: string, locationSlug: string) => {
  const slugByRoomType: Record<string, string> = {
    "Single AC Room": "single-ac-room",
    "Double Sharing": "double-sharing-room",
    "Triple Sharing": "triple-sharing-room",
  };

  return `/properties/${slugByRoomType[roomType] ?? "single-ac-room"}?location=${encodeURIComponent(locationSlug)}`;
};

export function PgLocationPage({ data: fallbackData }: { data: PgLocationData }) {
  const data = useCmsLocationData(fallbackData.slug, fallbackData);
  const hiddenSections = useCmsHiddenSections(fallbackData.slug);
  const show = (sectionId: string) => !hiddenSections.has(sectionId);

  return (
    <SiteLayout>
      {show("hero") && <Hero data={data} />}
      {show("trust-strip") && <TrustStrip data={data} />}
      {show("property-listings") && <PgRooms data={data} />}
      {show("nearby-areas") && <AreasWeServe data={data} />}
      {show("amenities") && <Amenities />}
      {show("why-choose") && <WhyChoose />}
      {show("why-stay") && <WhyStay data={data} />}
      {show("landmarks") && <Landmarks data={data} />}
      {show("gallery") && <Gallery data={data} />}
      {show("testimonials") && <Reviews data={data} />}
      {show("seo-content") && <SeoContent data={data} />}
      {show("faqs") && <Faqs data={data} />}
      {show("map") && <Map data={data} />}
      {show("cta") && <FinalCta data={data} />}
    </SiteLayout>
  );
}
...
function Hero({ data }: { data: PgLocationData }) {
  ...
  const twin = useCmsTwinPage(`location:${data.slug}`);
          ...
          >
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-white/70 px-3 py-1 text-xs font-medium text-muted-foreground shadow-soft backdrop-blur">
              ...
              {twinValue(
                ...
                "content",
                "badge",
                `Managed PG - ${data.area} - ${data.city}`,
              )}
            </span>
            <h1 className="mt-4 font-display text-3xl font-bold leading-[1.08] tracking-tight sm:text-4xl md:text-5xl lg:text-6xl">
              {twinValue(twin, "hero", "content", "headline", data.headline)}
            </h1>
            <p className="mt-5 max-w-xl text-base text-muted-foreground md:text-lg">
              {twinValue(twin, "hero", "content", "subheadline", data.subheadline)}
            </p>

            <div className="mt-7 flex flex-wrap gap-3">
              <Link
                to={twinHref(twin, "hero", "content", "book", "/contact") as "/contact"}
                className="inline-flex items-center gap-2 rounded-full gradient-brand px-5 py-3 text-sm font-semibold text-white shadow-soft transition hover:shadow-lift"
              >
                {twinValue(
                  twin,
                  "hero",
                  "content",
                  "book",
                  `${settings.primaryCtaText} in ${data.area}`,
                )}{" "}
                <ArrowRight className="h-4 w-4" />
              </Link>
              <a
                href={twinHref(twin, "hero", "content", "whatsapp", settings.whatsappHref)}
                target="_blank"
                rel="noreferrer"
                className="group inline-flex items-center gap-2 rounded-full border border-border bg-background px-5 py-3 text-sm font-semibold transition hover:bg-accent"
              >
                <WhatsAppIcon className="h-4 w-4 text-muted-foreground transition group-hover:text-[#25D366]" />{" "}
                {twinValue(twin, "hero", "content", "whatsapp", settings.secondaryCtaText)}
              </a>
              <a
                href={twinHref(twin, "hero", "content", "call", settings.phoneHref)}
              ...
              >
                ...
                {twinValue(twin, "hero", "content", "call", `Call ${settings.phone}`)}
              </a>
            </div>
              ...
              {[
                ...
                <div key={s.label || s.labelSuffix}>
                  ...
                  <dd className="text-xs text-muted-foreground">
                    {s.labelValue !== undefined ? (
                      <AnimatedNumber value={s.labelValue} suffix={s.labelSuffix} />
                    ) : (
                      s.label
                    )}
                  </dd>
      ...
      <div className="pause-on-hover relative max-w-full overflow-hidden rounded-[2rem] border border-border bg-white shadow-lift">
        ...
        <div className="hero-carousel flex w-max gap-4 p-4">
          {loop.map((item, i) => (
            <div
              key={i}
              className="group relative h-[400px] w-[300px] shrink-0 overflow-hidden rounded-2xl shadow-soft md:h-[480px] md:w-[360px]"
            >
              <img
                src={item.src}
                alt={item.alt}
...
function TrustStrip({ data }: { data: PgLocationData }) {
  const items = [
    { icon: ShieldCheck, t: "Managed Properties" },
      ...
      <div className="rounded-3xl border border-border bg-card p-3 shadow-lift md:p-4">
        <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-5">
          {items.map((it) => (
            <div
              key={it.t}
              className="flex items-center gap-3 rounded-2xl bg-[color:var(--surface)] px-4 py-3"
            >
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-20">
      <SectionHead
        ...
        title={`Premium PGs in ${data.area}`}
        sub="Choose your room type — single, double or triple sharing. Every room is fully furnished and move-in ready."
      />
      ...
      <AnimatePresence>
        ...
        >
          {list.map((r, i) => (
            <motion.a
              key={r.type + i}
              href={propertyHrefForRoom(r.type, data.slug)}
              variants={{
                hidden: { opacity: 0, y: 14, scale: 0.98 },
                show: { opacity: 1, y: 0, scale: 1 },
              }}
              className="group flex flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-soft transition hover:-translate-y-1 hover:shadow-lift focus:outline-none focus:ring-2 focus:ring-[color:var(--brand)] focus:ring-offset-2"
              aria-label={`View ${r.type} property details`}
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                <img
                  src={r.image}
                  alt={`${r.type} PG in ${data.area}`}
                  loading="lazy"
                ...
                <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-foreground backdrop-blur">
                  <ShieldCheck className="h-3.5 w-3.5 text-[color:var(--brand)]" /> Available
                </span>
              </div>
                ...
                <div className="mt-auto flex items-end justify-between border-t border-border pt-3">
                  <div>
                    ...
                    <p className="font-display text-xl font-bold">
                      ₹{r.priceFrom.toLocaleString("en-IN")}
                      <span className="text-sm font-medium text-muted-foreground">/mo</span>
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-semibold text-emerald-600">
                      {r.available} beds available
                    </p>
    ...
    <section className="bg-[color:var(--surface)] py-16 md:py-20">
      <div className="mx-auto max-w-7xl px-5">
        <SectionHead
          ...
          sub="Explore fully furnished premium PG rooms across this area with premium amenities, excellent connectivity, and zero brokerage."
        />
        ...
        >
          {visibleAreas.map((area) => (
            ...
            >
              ...
              <span className="relative min-w-0 truncate text-sm font-semibold text-foreground">
                {area.name}
              </span>
            </motion.a>
          ))}
  ...
  const items = [
    {
      icon: IndianRupee,
      t: "Zero Brokerage",
      d: "Move in without paying any brokerage. Pay only deposit and rent.",
    },
    {
      icon: ShieldCheck,
      t: "Managed Properties",
      d: "Every property is reviewed by the MyRoomiee team for comfort and service readiness.",
    },
    {
      icon: MapPin,
      t: "Prime Locations",
      d: "Walking distance from stations, offices and colleges.",
    },
    {
      icon: BedDouble,
      t: "Fully Furnished",
      d: "Bed, mattress, wardrobe, AC and WiFi — ready to use.",
    },
    {
      icon: Sparkles,
      t: "Professional Housekeeping",
      d: "Daily cleaning, fresh linen and trained staff.",
    },
    {
      icon: Camera,
      t: "CCTV & Safe Living",
      d: "24x7 CCTV, biometric access and resident support.",
    },
    {
      icon: Wifi,
      t: "High-speed WiFi",
      d: "Stable WiFi across rooms — perfect for WFH and study.",
    },
    { icon: Droplet, t: "Water Purifier", d: "Clean, filtered drinking water on every floor." },
  ];
  return (
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-20">
      <SectionHead
        ...
        sub="Each room and common area is loaded with premium amenities — no hidden costs."
      />
      <div className="mt-10 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-7">
        {amenitiesGrid.map((a, amenityIndex) => (
          <div
            key={`${a.name}-${amenityIndex}`}
            className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-border bg-card p-4 text-center shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift"
          >
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-20">
      <SectionHead
        ...
        title={`What's around our ${data.area} PGs`}
        sub="Distance and travel time from MyRoomiee properties."
      />
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-[color:var(--surface-muted)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                  {l.type}
                </span>
              </div>
              <h3 className="mt-4 font-display text-base font-semibold">{l.name}</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {l.distanceKm} km • {l.minutes} min
              </p>
            </div>
          );
        })}
  ...
  return (
    <section className="bg-[color:var(--surface)] py-20">
      <div className="mx-auto max-w-7xl px-5">
        <SectionHead
          eyebrow="Gallery"
          title={`Inside our ${data.area} properties`}
          sub="Real photos of bedrooms, study areas, lounges, dining and washrooms."
        />
...
function Reviews({ data }: { data: PgLocationData }) {
  ...
  const loop = useMemo(
    () => [...data.testimonials, ...data.testimonials, ...data.testimonials],
    [data.testimonials],
  );
    ...
    >
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <SectionHead
          ...
          title={`Loved by residents in ${data.area}`}
          sub={`${data.googleRating}★ average • ${data.googleReviews}+ Google reviews`}
        />
        ...
        >
          {loop.map((t, i) => (
            <div
              key={`${t.name}-${i}`}
              className="px-2"
              style={{ flex: `0 0 ${100 / loop.length}%` }}
            >
              <article className="group flex h-full min-h-[280px] flex-col rounded-3xl border border-border bg-card p-6 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-[color:var(--brand)]/35 hover:shadow-lift">
                ...
                <div className="mt-6 flex items-center gap-2 border-t border-border pt-4 text-xs font-semibold text-[color:var(--brand)]">
                  <CheckCircle2 className="h-4 w-4" /> Resident review
                </div>
              </article>
            </div>
          ))}
...
function SeoContent({ data }: { data: PgLocationData }) {
  ...
  return (
    <section
      className="bg-[color:var(--surface)] py-20"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
    >
      <div className="mx-auto max-w-7xl px-5">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <SectionHead
            ...
            sub="Everything you need to know before choosing your next PG or premium PG room."
          />
          <div className="flex gap-2">
            <button
              type="button"
        ...
        <div className="mt-10 overflow-hidden">
          <motion.div
            animate={{ x: `-${active * slideStep}%` }}
            transition={
              animateTrack ? { duration: 0.55, ease: [0.22, 1, 0.36, 1] } : { duration: 0 }
            }
          ...
          >
            {loop.map((card, i) => (
              <div
                ...
                style={{ flex: `0 0 ${100 / loop.length}%` }}
              >
                <motion.article className="group h-full overflow-hidden rounded-3xl border border-border bg-card shadow-soft transition duration-300 hover:-translate-y-1 hover:border-[color:var(--brand)]/35 hover:shadow-lift">
                  ...
                  <div className="p-5">
                    ...
                    <p className="mt-2 min-h-[60px] text-sm leading-relaxed text-muted-foreground">
                      {card.body}
                    </p>
                    <a
                      href={`/${data.slug}#faq`}
                      className="mt-5 inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-3.5 py-2 text-xs font-semibold text-foreground shadow-soft transition hover:border-[color:var(--brand)]/40 hover:bg-[color:var(--brand-soft)] hover:text-[color:var(--brand)] hover:shadow-lift"
                    >
                      Read More{" "}
...
function Faqs({ data }: { data: PgLocationData }) {
  ...
  return (
    <section id="faq" className="mx-auto max-w-4xl px-5 py-20">
      <SectionHead
        eyebrow="FAQ"
        title={`PG in ${data.area} — frequently asked questions`}
        sub="Everything you wanted to know before booking your stay."
      />
...
function Map({ data }: { data: PgLocationData }) {
  return (
    <section className="mx-auto max-w-7xl px-5 py-20">
      <SectionHead
        eyebrow="Property Coverage"
        title={`Find us across ${data.area}`}
        sub="Our managed properties are spread across the most preferred streets and societies in the area."
      />
          ...
          <div className="p-8 md:p-12">
            <h2 className="font-display text-3xl font-bold md:text-4xl">
              {twinValue(
                ...
                "content",
                "heading",
                `Looking for the best PG in ${data.area}?`,
              )}
            </h2>
            <p className="mt-3 max-w-md text-muted-foreground">
              {twinValue(
                ...
                "content",
                "description",
                `Book a free visit today. Our ${data.area} property manager will walk you through available rooms and amenities.`,
              )}
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Link
              ...
              >
                {twinValue(
                  ...
                  "content",
                  "book",
                  `${settings.primaryCtaText} ${data.area} Visit`,
                )}
              </Link>
              <a
              ...
              >
                <Phone className="h-4 w-4" />{" "}
                {twinValue(twin, "cta", "content", "call", settings.phone)}
              </a>
            </div>
            <div className="mt-8 flex flex-wrap items-center gap-3 text-xs">
              {["Managed", "Zero Brokerage", "Daily Housekeeping", "CCTV Secured", "Furnished"].map(
                (b) => (
                  <span
                    key={b}
                    className="inline-flex items-center gap-1 rounded-full border border-border bg-[color:var(--surface)] px-2.5 py-1 font-semibold text-muted-foreground"
                  >
            ...
            <div className="mt-6 flex items-start gap-2 text-xs text-muted-foreground">
              ...
              <span>{twinValue(twin, "cta", "content", "address", settings.address)}</span>
            </div>
          </div>
...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-3.5 w-3.5">
      <path
        fill="#4285F4"
        d="M22.6 12.2c0-.8-.1-1.5-.2-2.2H12v4.2h5.9a5 5 0 0 1-2.2 3.3v2.7h3.6c2.1-1.9 3.3-4.7 3.3-8z"
      />
      <path
        fill="#34A853"
        d="M12 23c3 0 5.5-1 7.3-2.7l-3.6-2.7c-1 .7-2.2 1.1-3.7 1.1-2.8 0-5.2-1.9-6.1-4.5H2.2V17A11 11 0 0 0 12 23z"
      />
      <path fill="#FBBC05" d="M5.9 14.2a6.7 6.7 0 0 1 0-4.3V7.1H2.2a11 11 0 0 0 0 9.8l3.7-2.7z" />
      <path
        fill="#EA4335"
        d="M12 5.3c1.6 0 3.1.6 4.2 1.7l3.2-3.2A10.8 10.8 0 0 0 12 1 11 11 0 0 0 2.2 7.1l3.7 2.8C6.8 7.2 9.2 5.3 12 5.3z"
      />
    </svg>
  );
}
