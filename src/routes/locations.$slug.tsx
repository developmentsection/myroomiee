...
export const Route = createFileRoute("/locations/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData ? [
      { title: `PG & Coliving in ${loaderData.loc.name}, Mumbai — MyRoomiee` },
      { name: "description", content: `Premium fully furnished PG and coliving in ${loaderData.loc.name}, Mumbai. Zero brokerage, AC rooms, WiFi, meals.` },
    ...
    <SiteLayout>
      <section className="border-b border-border bg-[color:var(--surface)]">
        <div className="mx-auto max-w-5xl px-5 py-16">
          ...
          <h1 className="mt-2 font-display text-4xl font-bold md:text-5xl">PG & Coliving in {loc.name}</h1>
          <p className="mt-4 max-w-3xl text-muted-foreground">Premium fully furnished PG in {loc.name} — Boys, Girls and unisex coliving with AC, WiFi, housekeeping and meals. Zero brokerage.</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16">
        <div className="grid gap-5 md:grid-cols-3">
          {[
            ...
            { icon: Sparkles, t: "Premium amenities", d: "AC, WiFi, CCTV, housekeeping and meals included." },
          ].map((b) => (
